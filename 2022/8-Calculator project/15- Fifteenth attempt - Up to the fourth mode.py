...
def checker_only_number():
    get_number = lbl_numbers_and_operator['text']
    for checker in get_number:
        if checker == '+':
            return
        elif checker == '-':
            return
            ...
            lbl_alarm_for_empty['text'] = get_number + '= !!!'
...
def put_sign_after_number(new_operator):
    get_number = lbl_numbers_and_operator['text']
    if get_number[-1] == '+' or get_number[-1] == '-' or get_number[-1] == '/' or get_number[-1] == '*':
        get_number = get_number[:-1] + new_operator
        lbl_alarm_for_empty['text'] += get_number + ' '
    else:
        lbl_numbers_and_operator['text'] = ''
        get_number += new_operator
        lbl_alarm_for_empty['text'] += get_number + ' '
